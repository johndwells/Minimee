...
class Minimee_lib {
	...
	public $cache_hash_method		= '';		// a hash of settings & filenames
	...
	protected function _create_cache_name($name)
	{
		...
		switch($this->config->hash_method) :
			...
			case 'sanitize' :
				...
				// now sanitise
				$this->cache_hash_method = $this->EE->security->sanitize_filename($name);

				// reduce length to be safe?
				if(strlen($this->cache_hash_method) > 200)
				{
					$this->cache_hash_method = substr($this->cache_hash_method, 0, 200);
				}
			break;

			case 'md5' :
				$this->cache_hash_method = md5($name);
			break;

			default :
			case 'sha1' :
				$this->cache_hash_method = sha1($name);
			break;
		...
		// put it all together
		return $this->cache_hash_method . '.' . $this->cache_lastmodified . $cachebust . '.' . $this->type;
	}
	// ------------------------------------------------------
		...
		if ($this->config->is_yes('cleanup'))
		{
			if ($handle = opendir($this->config->cache_path))
			{
				while (false !== ($file = readdir($handle)))
				{
					...
					if (strpos($file, $this->cache_hash_method) === 0)
					{
